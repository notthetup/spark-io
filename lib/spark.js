...
function Spark(opts) {
  ...
  this.connect(function(error, data) {
    ...
    if (error !== undefined && error !== null) {
      ...
      console.log('connected via', data.type, 'to', state.host +':'+ state.port)
...
Spark.prototype.connect = function(handler) {
  ...
  this.connectViaMDNS(function(err, data){
    if (err){
      if(sparkCloudError){
        if (handler){
          handler(err, data);
    ...
    }else if (!state.isConnected && handler){
      handler(err,data)
    }
  });

  this.connectViaSparkCloud(function(err, data){
   if (err){
      if(mdnsError){
        if (handler){
          handler(err, data);
...
Spark.prototype.connectViaMDNS = function(handler){
  var state = priv.get(this);
  var firmataService = '_firmata._tcp.local'
  mdns.query({
    questions:[{
      name: firmataService,
      type: 'PTR'
  ...
  function parseMDNSResponse (response) {
    if (response.answers && response.answers[0]){
      if (response.answers[0].data === ''+state.deviceId+'.'+firmataService){
        var mdnsData = {};
        var dnsResponseAdditionals = response.additionals || [];
        dnsResponseAdditionals.forEach(function(thisAdd){
         if (thisAdd.type === 'SRV') {
           mdnsData.port = thisAdd.data.port;
         }else if(thisAdd.type === 'A'){
           mdnsData.host = thisAdd.data;
         }
       });
        if (mdnsData.host && mdnsData.port && handler){
          mdnsData.type = 'mdns';
          handler(null, mdnsData);
        }
      }
    }
  }
  mdns.on('response',parseMDNSResponse);

  setTimeout(function(){
    mdns.removeListener('response',parseMDNSResponse);
    if (!state.isConnected){
      handler(new Error(errors.timeout), null);
    }
  },32000)
}

Spark.prototype.connectViaSparkCloud = function(handler){
  var state = priv.get(this);
    ...
    res.on("end", function () {
      if (res.statusCode === 200) {
          ...
          if (data.error) {
            ...
            returnData.type = "spark";
            var address = data.result.split(":");
            returnData.host = address[0];
            returnData.port = parseInt(address[1], 10);
          }
          if (handler) {
            handler(err, returnData);
          }
      } else {
        var data = JSON.parse(body);
          err = data.error || errors.cloud + ": code: " + res.statusCode;
        if (handler) {
